...
This article describes parts of the current design and implementation of the Ni language. It's not a tutorial, introduction or manual. It's in fact kinda incoherent - but so is Ni :)

## The parser

The [Ni parser](https://github.com/gokr/ni/blob/master/src/niparser.nim) is a straight forward hand made <del>recursive descent</del> parser. Hmmm, now that I look closer I realize its not recursive! Its just a very simple [while loop character by character](https://github.com/gokr/ni/blob/master/src/niparser.nim#L477) and maintaining some state. It should be easy to read and understand and given that the grammar is so simple, it hasn't changed much lately. It could probably be made faster by using more procs and less methods, but it's probably plenty fast since the grammar is so simple.

The parser constructs an AST consisting of instances of [subclasses of Node](https://github.com/gokr/ni/blob/master/src/niparser.nim#L27-L79). A Node is a regular "ref object" which is Nim terminology for a GC controlled object allocated on the heap. Its worth noting that the parser can be used alone without the interpreter, but the [ni module](https://github.com/gokr/ni/blob/master/src/ni.nim) (the interpreter) obviously depends on the parser module.
...
The values **true**, **false** and **nil** are represented with three singleton Values that are held by the interpreter. Those three singletons are bound to the words `true`, `false` and `nil` in the root context, which is the global namespace.
...
I have been mulling these rules over, but I think this logic would make it fairly reasonable.
